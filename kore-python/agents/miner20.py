...
# refacto from miner_17
# real processing of core encounter on the route
...
def launch_new_shipyard(shipyard):
...
def agent(obs, config):
    ...
    for i in l:
        ...
        if(remainingKore > 100 and shipyard.max_spawn > 5):
            ...
            if(shipyard.ship_count >= 70 and can_convert(board)):
                launch_new_shipyard(shipyard)

            elif(remainingKore >= spawn_cost and turn < 350):
                nb_ships_to_spawn = min(
                    shipyard.max_spawn, math.floor(remainingKore / spawn_cost))
                remainingKore -= spawn_cost * nb_ships_to_spawn
        ...
        elif(remainingKore > board.configuration.spawn_cost * shipyard.max_spawn and turn < 350):
            nb_spawn = min(shipyard.max_spawn, math.floor(
                remainingKore / spawn_cost))
            remainingKore -= board.configuration.spawn_cost*nb_spawn
            action = ShipyardAction.spawn_ships(nb_spawn)
            shipyard.next_action = action
